...
# Удаление аномальных значений и выбросов в повторяющихся камнях
...
# Группировка и выбор последней по времени цены
...
# Объединение результатов в один чистый датасет
...
# Считаем среднюю цену за карат для каждой группы в каждом месяце
df_index = df_cleaned.groupby(['date'] + categories)['price_per_carat'].mean().reset_index()

# Находим максимальную (последнюю доступную) дату в датасете
date_max = df_index['date'].max()
print(f"Базовый месяц для расчета инфляции: {date_max}")

# Выделяем цены на эту максимальную дату, чтобы сделать их базой (знаменателем)
...
# Объединяем базовые цены с историческим индексом
df_index = df_index.merge(df_base_prices, on=categories, how='outer')

# Считаем коэффициент инфляции (отношение базовой цены к цене в конкретном месяце)
...
# Объединяем полученные коэффициенты инфляции с нашей основной очищенной таблицей
df_with_inf = df_cleaned.merge(df_index[['date'] + categories + ['inflation']], on=['date'] + categories, how='left')

# Создаем скорректированную целевую переменную
